...
from patchright._impl._js_handle import Serializable, serialize_argument
...
class Locator:
    ...
    async def drop(
        self, payload: DropPayload, position: Position = None, timeout: float = None
    ) -> None:
        await self._frame._drop(
            self._selector, payload, strict=True, position=position, timeout=timeout
        )
...
def get_by_role_selector(
    ...
    if description is not None:
        props.append(
            ("description", escape_for_attribute_selector(description, exact=exact))
        )
