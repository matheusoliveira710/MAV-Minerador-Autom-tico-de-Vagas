...
_PLAYWRIGHT_MODULE_PATH = str(Path(patchright.__file__).parents[0])
    ...
    def send_no_reply(
        ...
        self._connection.wrap_api_call_sync(
            lambda: self._connection._send_message_to_server(
                self._object, method, augmented_params, timeout, True
            ),
            is_internal,
            title,
        )
    ...
    async def _inner_send(
        ...
        try:
            ...
            await self._connection._abort(
                self._object, callback, str(exc) or "Task was cancelled"
            )
            raise
...
class Connection(EventEmitter):
    ...
    async def _abort(
        ...
        try:
                ...
                {self._transport.on_error_future, callback.future},
                return_when=asyncio.FIRST_COMPLETED,
            )
        finally:
            if not callback.future.done():
...
def _capture_stack_trace() -> ParsedStackTrace:
    ...
    while frame:
        ...
        if patchright._impl._impl_to_api_mapping.__file__ == filename:
            frame = frame.f_back
            continue
    ...
    if not api_name:
        api_name = last_internal_api_name
    return {"frames": parsed_frames, "apiName": api_name, "title": None}


def _attach_api_call_information(
    stack_trace_information: ParsedStackTrace, is_internal: bool, title: Optional[str]
) -> ParsedStackTrace:
    if is_internal:
